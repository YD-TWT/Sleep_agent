...
class FinalRefer(Node):
    ...
    def get_spatial_info(self):
        return super().get_spatial_info(for_decision=True)

    def get_temporal_info(self):
        return super().get_temporal_info(for_decision=True)
    ...
    def _process_inputs(self, raw_inputs:Dict[str,str], spatial_info:Dict[str,Any], temporal_info:Dict[str,Any], **kwargs)->List[Any]:
        ...
        spatial_str = ""
        for id, info in spatial_info.items():
            spatial_str += id + ": " + str(info['output']) + "\n\n"
    ...
    async def _async_execute(self, input:Dict[str,str],  spatial_info:Dict[str,Any], temporal_info:Dict[str,Any],**kwargs):
        ...
        message = [{'role':'system','content':system_prompt},{'role':'user','content':user_prompt}]
        response = await self.llm.agen(message)
        agent_debug_print(f"################system prompt:{system_prompt}")
        agent_debug_print(f"################user prompt:{user_prompt}")
