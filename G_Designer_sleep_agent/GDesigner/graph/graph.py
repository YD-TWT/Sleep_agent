...
class Graph(ABC):
    ...
    def connect_decision_node(self, active_node_ids: Optional[set] = None):
        for node_id in self.nodes.keys():
            if active_node_ids is not None and node_id not in active_node_ids:
                continue
            self.nodes[node_id].add_successor(self.decision_node)

    def _agent_spatial_successors(self, node: Node) -> List[Node]:
        return [succ for succ in node.spatial_successors if succ.id in self.nodes]

    def _is_isolated_node(self, node_id: str) -> bool:
        node = self.nodes[node_id]


        return (
            len(node.spatial_predecessors) == 0
            and len(self._agent_spatial_successors(node)) == 0
        )
    ...
    def run(self, inputs: Any,
        ...
        for round in range(num_rounds):
            ...
            while zero_in_degree_queue:
                ...
                if self._is_isolated_node(current_node_id):
                    continue
        ...
        self.connect_decision_node(active_node_ids=active_node_ids)
    ...
    async def arun(self, input: Dict[str,str],
        ...
        active_node_ids: set = set()
        for round in range(num_rounds):
            log_probs += self.construct_spatial_connection(threshold=threshold)
            log_probs += self.construct_temporal_connection(round, threshold=threshold)
